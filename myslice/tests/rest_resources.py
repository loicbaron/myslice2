...
class TestResources(LocalTestCase):
    ...
    def test_0_getNoAuth(self):
        r = requests.get('http://'+server+':8111/api/v1/resources')
        self.assertEqual(r.status_code, 400)

    def test_1_getAllResources(self):
        r = requests.get('http://'+server+':8111/api/v1/resources', cookies=self.cookies)
    ...
    def test_2_getTestbedResources(self):
        ...
        r = requests.get('http://'+server+':8111/api/v1/testbeds/'+testbed+'/resources', cookies=self.cookies)
        self.assertEqual(r.status_code, 200)
