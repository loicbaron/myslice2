...
class TestProjects(LocalTestCase):
    ...
    def test_0_getNoAuth(self):
        r = requests.get('http://'+self.server+'/api/v1/projects')
        self.assertEqual(r.status_code, 400)

    def test_0_postNoAuth(self):
        r = requests.post('http://'+self.server+'/api/v1/projects')
        self.assertEqual(r.status_code, 400)

    def test_0_putNoAuth(self):
        r = requests.put('http://'+self.server+'/api/v1/projects')
        self.assertEqual(r.status_code, 400)

    def test_1_getProjects(self):
        r = requests.get('http://'+self.server+'/api/v1/projects', cookies=self.cookies)
    ...
    #    r = requests.post('http://'+self.server+'/api/v1/projects', headers={str('Content-Type'):'application/json'}, data=json.dumps(payload), timeout=self.timeout)
    #    self.assertEqual(r.status_code, 200)
    ...
    def test_3_getProjectId(self):
        ...
        # print(id)
        r = requests.get('http://'+self.server+'/api/v1/projects/'+id, cookies=self.cookies)
        #pprint(r.text)
        self.assertEqual(r.status_code, 200)
        res = json.loads(r.text)
    ...
    def test_4_putProject(self):
        ...
        rGet = requests.get('http://'+self.server+'/api/v1/projects/'+id, cookies=self.cookies)
        ...
        rPut = requests.put('http://'+self.server+'/api/v1/projects/'+id, headers={str('Content-Type'):'application/json'}, data=json.dumps(payload), cookies=self.cookies, timeout=self.timeout)
        pprint(rPut.text)
        ...
        rUpdated = requests.get('http://'+self.server+'/api/v1/projects/'+id, cookies=self.cookies)
    ...
    def test_5_deleteProject(self):
        ...
        if not id:
            self.assertEqual(id, "Project was not created in previous test, we cannot continue this test")
        rDelete = requests.delete('http://'+self.server+'/api/v1/projects/'+id, cookies=self.cookies)
        pprint(rDelete.text)
