...
class TestSlices(LocalTestCase):
    ...
    def test_0_getNoAuth(self):
        r = requests.get('http://'+self.server+'/api/v1/slices')
        self.assertEqual(r.status_code, 400)

    def test_0_postNoAuth(self):
        r = requests.post('http://'+self.server+'/api/v1/slices')
        self.assertEqual(r.status_code, 400)

    def test_0_putNoAuth(self):
        r = requests.put('http://'+self.server+'/api/v1/slices')
        self.assertEqual(r.status_code, 400)

    def test_1_getAllSlices(self):
        r = requests.get('http://'+self.server+'/api/v1/slices', cookies=self.cookies)
    ...
    def test_1_getProjectSlices(self):
        ...
        r = requests.get('http://'+self.server+'/api/v1/projects/'+project+'/slices', cookies=self.cookies)
    ...
    def test_3_getSliceId(self):
        ...
        rGet = requests.get('http://'+self.server+'/api/v1/slices/'+id, cookies=self.cookies)
    ...
    def test_4_putSliceUser(self):
        ...
        rPut = requests.put('http://'+self.server+'/api/v1/slices/'+id, headers={str('Content-Type'):'application/json'}, data=json.dumps(slice), cookies=self.cookies, timeout=self.timeout)
        pprint(rPut.text)
        ...
        rUpdated = requests.get('http://'+self.server+'/api/v1/slices/'+id, cookies=self.cookies)
    ...
    #    r = requests.get('http://'+self.server+'/api/v1/testbeds/'+testbeds[0]+'/resources', cookies=self.cookies)
    #    self.assertEqual(r.status_code, 200)
    ...
    def test_7_deleteSlice(self):
        ...
        if not id:
            self.assertEqual(id, "expected created_slice but got none")
        rDelete = requests.delete('http://'+self.server+'/api/v1/slices/'+id, cookies=self.cookies)
        pprint(rDelete.text)
