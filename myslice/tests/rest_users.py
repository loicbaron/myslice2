...
class TestUsers(LocalTestCase):
    ...
    def test_0_getNoAuth(self):
        r = requests.get('http://'+self.server+'/api/v1/users')
        self.assertEqual(r.status_code, 400)

    def test_0_postNoAuth(self):
        r = requests.post('http://'+self.server+'/api/v1/users')
        self.assertEqual(r.status_code, 400)

    def test_0_putNoAuth(self):
        r = requests.put('http://'+self.server+'/api/v1/users')
        self.assertEqual(r.status_code, 400)


    def test_1_getUsers(self):
        r = requests.get('http://'+self.server+'/api/v1/users', cookies=self.cookies)
        self.assertEqual(r.status_code, 200)

    def test_2_postWrongUser(self):
        payload = {}
        r = requests.post('http://'+self.server+'/api/v1/users', headers={str('Content-Type'):'application/json'}, data=json.dumps(payload), cookies=self.cookies, timeout=self.timeout)
        self.assertEqual(r.status_code, 400)
    ...
    def test_3_postUserNoAuth(self):
        ...
        r = requests.post('http://'+self.server+'/api/v1/users', headers={str('Content-Type'):'application/json'}, data=json.dumps(payload), timeout=self.timeout)
        pprint(r.text)
        ...
        for event in result['events']:
            ...
            rConfirm = requests.get('http://'+self.server+'/confirm/'+event)
            ...
            rRequest = requests.put('http://'+self.server+'/api/v1/requests/'+event, headers={str('Content-Type'):'application/json'}, data=json.dumps(deny), cookies=self.cookies)
            print(rRequest.text)
    ...
    def test_5_getUserId(self):
        ...
        if id:
            r = requests.get('http://'+self.server+'/api/v1/users/'+id, cookies=self.cookies)
            self.assertEqual(r.status_code, 200)
            res = json.loads(r.text)
    ...
    def test_6_putUser(self):
        id = self.__class__.created_user
        if id:
            rGet = requests.get('http://'+self.server+'/api/v1/users/'+id, cookies=self.cookies)
            ...
            rPut = requests.put('http://'+self.server+'/api/v1/users/'+id, headers={str('Content-Type'):'application/json'}, data=json.dumps(payload), cookies=self.cookies, timeout=self.timeout)
            pprint(rPut.text)
            ...
            rUpdated = requests.get('http://'+self.server+'/api/v1/users/'+id, cookies=self.cookies)
    ...
    def test_7_deleteUser(self):
        ...
        if id:
            ...
            rDelete = requests.delete('http://'+self.server+'/api/v1/users/'+id, cookies=self.cookies)
            pprint(rDelete.text)
