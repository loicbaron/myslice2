...
class TestUsers(Tests):
    ...
    def test_0_postNoAuth(self):
        r = requests.post('http://localhost:8111/api/v1/users')
        # user not authenticated
        #pprint(r.text)
        self.assertEqual(r.status_code, 400)

    def test_0_putNoAuth(self):
        r = requests.put('http://localhost:8111/api/v1/users')
        # user not authenticated
        #pprint(r.text)
        self.assertEqual(r.status_code, 400)
    ...
    def test_1_getUsers(self):
        r = requests.get('http://localhost:8111/api/v1/users', cookies=self.cookies)
        #pprint(r.text)
        self.assertEqual(r.status_code, 200)

    def test_2_postWrongUser(self):
        payload = {}
        r = requests.post('http://localhost:8111/api/v1/users', headers={str('Content-Type'):'application/json'}, data=json.dumps(payload), cookies=self.cookies, timeout=self.timeout)
        #pprint(r.text)
    ...
    def test_2_postUserNoAuth(self):
        ...
        r = requests.post('http://localhost:8111/api/v1/users', headers={str('Content-Type'):'application/json'}, data=json.dumps(payload), timeout=self.timeout)
        self.assertEqual(r.status_code, 200)
    ...
    def test_3_getUserId(self):
        ...
        print(id)
        r = requests.get('http://localhost:8111/api/v1/users/'+id, cookies=self.cookies)
        #pprint(r.text)
        self.assertEqual(r.status_code, 200)

    def test_4_putUser(self):
        id = self.__class__.created_user
        rGet = requests.get('http://localhost:8111/api/v1/users/'+id, cookies=self.cookies)
        ...
        rPut = requests.put('http://localhost:8111/api/v1/users/'+id, headers={str('Content-Type'):'application/json'}, data=json.dumps(payload), cookies=self.cookies, timeout=self.timeout)
        pprint(rPut.text)
        ...
        rUpdated = requests.get('http://localhost:8111/api/v1/users/'+id, cookies=self.cookies)
    ...
    def test_5_deleteUser(self):
        id = self.__class__.created_user
        rDelete = requests.delete('http://localhost:8111/api/v1/users/'+id, cookies=self.cookies)
        pprint(rDelete.text)
